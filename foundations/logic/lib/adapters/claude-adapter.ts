...
export class ClaudeAdapter extends BasePlatformAdapter {
  name = 'claude';
  private outputDir = '.claude/rules';
  ...
  format(rules: Rule[]): string {
    ...
    const claudeRules = rules.filter(r =>
      r.enabled !== false &&
   ...
   * 카테고리별로 여러 파일 생성
   */
  async syncMultiple(rules: Rule[]): Promise<{ files: string[]; errors: string[] }> {
    const claudeRules = rules.filter(r =>
      r.enabled !== false &&
      (r.platforms?.claude?.enabled !== false || !r.platforms)
    );
  ...
  private formatIndex(rules: Rule[]): string {
    const lines: string[] = [];
  ...
  private formatCategory(category: string, rules: Rule[]): string {
    const lines: string[] = [];
  ...
  private formatRuleForClaude(rule: Rule): string {
    const lines: string[] = [];
  ...
  private formatAllRules(rules: Rule[]): string {
    const lines: string[] = [];
  ...
  private getCategoryGlobs(category: string): string {
